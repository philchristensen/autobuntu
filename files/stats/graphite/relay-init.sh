...
# 	user that owns the /opt/graphite/storage/ folder. Carbon-relay will be invoked as that
# 	username on start.
#
...
#Carbon has its own logging facility, by default in /opt/graphite/storage/log/carbon-relay-*

# Path to Executable
DAEMON=$GRAPHITE_HOME/bin/carbon-relay.py

# NOTE: This is a hard-coded PID file, based on carbon-relay.py. If you have more the one carbon-relay
#	instance running on this machine, you'll need to figure out a better way to calculate the PID file.
PIDFILE=/opt/graphite/storage/carbon-relay-a.pid
